...
| 7 | **GitOps**: builds.sr.ht `.build.yml` + on-LAN deploy executor (see [gitops](gitops.md)) | 🟡 built 2026-08-22 — CI manifest (`.build.yml`, secret-free lint+syntax-check) + `ops/gitops_executor` on util01 (ansible venv + `deploy.sh` + poll timer, disabled). **Awaiting activation:** enable builds.sr.ht for the repo; clone repo + place vault-pass & ansible key on util01; flip `gitops_timer_enabled` (and `gitops_auto_apply` when ready). Trigger is a systemd poll timer, not an inbound webhook (cloud CI can't reach LAN n8n). |
