...
| 6 | **Maintenance**: `ops/restic` → nas02 (SFTP) — scoped to **Paperless + Syncthing** only; `--tags patch` play; Uptime Kuma verification | 🟡 built (2026-08-22): Syncthing live on util01; `ops/restic` + patch play written & wired. **Awaiting manual nas02 prep** (backup user/SSH + `vault_restic_password`) before first `--tags backup` run. |
